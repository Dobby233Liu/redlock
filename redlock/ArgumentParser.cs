...
internal abstract class ArgumentsBase
{
	...
	public ArgumentsBase(IEnumerable<string> args)
	{
		...
		var argsByOption = new Dictionary<string, List<string>>();
		...
		foreach (var entry in argsByOption.OrderBy(x => x.Key))
		{
			...
			if (behavior.MaxExtraParams >= 0 && entry.Value.Count > behavior.MaxExtraParams)
				throw new ArgumentException(
					$"Option {entry.Key} takes at most {behavior.MaxExtraParams} parameters, {entry.Value.Count} given");
	...
	private static bool TryParseOptionName(string arg, out string option)
	{
		...
		{
			return arg.StartsWith(prefix) && arg.Length > prefix.Length;
		}

		string ParseName(string arg, string prefix)
		{
			return arg.Substring(prefix.Length).ToLower();
		}
